...
{
    BeatDetector* detector = (BeatDetector*)malloc(sizeof(BeatDetector));
    if (detector) {
        beat_detector_init(detector);
    }
    return detector;
}
...
void beat_detector_update(BeatDetector* detector, const float* samples, int num_samples)
{
    ...
    *detector->history_ptr = energy;
    ...
    detector->average_energy = sum_energy / (float)BEAT_DETECTOR_HISTORY_SIZE;

    float variance_sum = 0.0f;
    for (size_t i = 0; i < BEAT_DETECTOR_HISTORY_SIZE; ++i) {
        variance_sum += (detector->history[i] - detector->average_energy) * (detector->history[i] - detector->average_energy);
    }
    detector->variance = variance_sum / (float)BEAT_DETECTOR_HISTORY_SIZE;
    ...
    if (detector->threshold < 1.0f) {
        detector->threshold = 1.0f;
    }
    if (detector->threshold > 3.0f) {
        detector->threshold = 3.0f;
    }
}
